...
basedir = '/logs/nerf_synthetic'               # where to store ckpts and logs
...
data = dict(
    ...
    testskip=8,                   # subsample testset to preview results
...
coarse_train = dict(
    ...
    lrate_density_std=1e-2,       # lr of density std voxel grid
    lrate_density_params=1e-2,    # lr of density params voxel grid
    ...
    weight_entropy_den=0.001,     # weight of entropy loss on density in variational inference
    weight_entropy_rgb=0.001,     # weight of entropy loss on rgb in variational inference
    weight_nearclip=0,
    weight_distortion=0,
...
fine_train.update(dict(
    ...
    weight_entropy_den=0.001,
    weight_entropy_rgb=0.001,
    weight_rgbper=0.01,
    weight_density=1e-2,          # weight of density regularizer loss
    lrate_k0=1e-2, # very useful !!
    pg_scale=[1000, 2000, 3000, 4000],
    skip_zero_grad_fields=['density', 'density_std', 'k0'],
))
