...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.uncertainty_grid = grid.create_grid(
            ...
            config=self.density_config)
        with torch.no_grad():
            self.uncertainty_grid.grid.data += 5
    ...
    def maskout_near_cam_vox(self, cam_o, near_clip):
        ...
        # update voxels that between cameras and their near planes
        self.uncertainty_grid.grid[nearest_dist[None,None] <= near_clip] = 0
        print('uncertainty_grid updated for near camera: ',(self.uncertainty_grid.grid.data==0).sum()/torch.prod(self.world_size))
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=-1, N_iters=None, **render_kwargs):
        '''Volume rendering
        ...
        if render_kwargs.get('update_uncertainty', False):
            # query for alpha w/ post-activation
            density = self.density(ray_pts)
            alpha = self.activate_density(density, interval)

            # compute accumulated transmittance
            weights, alphainv_last, T = Alphas2Weights.apply(alpha, ray_id, N)
            # update V_H(X_E,X_S)=0
            mask = (T>0.99)
        ...
        # compute accumulated transmittance
        weights, alphainv_last, _, _ = Alphas2Weights.apply(alpha, ray_id, N)
        if self.fast_color_thres > 0:
        ...
        if render_kwargs.get('compute_VH', False): 
            ...
            # compute uncertainty_VH for rays
            # for multiple points in a same voxel, we only compute once?? TODO
            # self.uncertainty_grid.grid[self.uncertainty_grid.grid==10] = 3
...
class DirectVoxGO1(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 num_voxels=0, num_voxels_base=0,
                 alpha_init=None,
                 mask_cache_path=None, mask_cache_thres=1e-3, mask_cache_world_size=None,
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        ray_pts = torch.zeros([100,128,3])
        shape = ray_pts.shape[:2]
        ray_id = torch.arange(shape[0]).view(-1,1).expand(shape).flatten()
        alpha = torch.ones_like(ray_id)*0.2
...
Stochastic Model
'''
class DirectVoxGO_Sto(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 num_voxels=0, num_voxels_base=0,
                 alpha_init=None,
                 mask_cache_path=None, mask_cache_thres=1e-3, mask_cache_world_size=None,
                 ...
                 rgbnet_dim=0, rgbnet_direct=False, rgbnet_full_implicit=False,
                 rgbnet_depth=3, rgbnet_width=128,
                 denfea_dim=16, rgbfea_dim=32,
                 viewbase_pe=4,
                 K_samples=12,
                 density_std_init=0,
                 **kwargs):
        super(DirectVoxGO_Sto, self).__init__()
        self.register_buffer('xyz_min', torch.Tensor(xyz_min))
        self.register_buffer('xyz_max', torch.Tensor(xyz_max))
        self.fast_color_thres = fast_color_thres
        ...
        self.rgbnet_full_implicit = rgbnet_full_implicit
        if rgbnet_dim <= 0:
            # color voxel grid  (coarse stage) model a distribution too
            self.k0_dim = 3 + 3 # uncertainty
            self.k0 = grid.create_grid(
                k0_type, channels=self.k0_dim, world_size=self.world_size,
                xyz_min=self.xyz_min, xyz_max=self.xyz_max,
                config=self.k0_config)
            ...
            self.rgbnet = nn.Sequential(
                ...
                *[
                ...
                # change the output layer as feature
        ...
        # Using the coarse geometry if provided (used to determine known free space and unknown space)
        # Re-implement as occupancy grid (2021/1/31)
        # mask_cache_path = None # train from scratch
        # print('dont use mask_cache from coarse model!!')
        self.mask_cache_path = mask_cache_path
        self.mask_cache_thres = mask_cache_thres
        if mask_cache_world_size is None:
            mask_cache_world_size = self.world_size
    ...
    @torch.no_grad()
    def update_uncertainty(self, rays_o, rays_d, viewdirs, global_step=-1, N_iters=-1, **render_kwargs):
        '''
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=-1, N_iters=-1, **render_kwargs):
        '''Volume rendering
        @rays_o:   [N, 3] the starting point of the N shooting rays.
        @rays_d:   [N, 3] the shooting direction of the N rays.
        @viewdirs: [N, 3] viewing direction to compute positional embedding for MLP.
        ...
        den_mean_k = den_mean[:,None].expand([BN, self.K_samples])
        den_std_k = den_std[:,None].expand([BN, self.K_samples])
        if render_kwargs.get('is_train', False):
            eps_den = torch.empty([1,self.K_samples]).normal_()
        else:
            eps_den = self.sample_den
        raw_density_k = eps_den.mul(den_std_k).add_(den_mean_k) # (BN,K)
        ...
        # new compute rgb on masked ray_pts
        k0 = self.k0(ray_pts)
        if self.rgbnet is None:
            # no view-depend effect
            # also model a distribution
            rgb_mean, rgb_std = k0[:,:3], k0[:,3:6]
        ...
        if render_kwargs.get('render_depth', False):
            with torch.no_grad():
                depth_k = segment_coo(
                        src=(weights_k * step_id.unsqueeze(-1)),
                        index=ray_id,
        ...
        if render_kwargs.get('compute_VH', False): 
            ...
            # compute uncertainty_VH for rays
            # for multiple points in a same voxel, we only compute once?? TODO
            self.uncertainty_grid.grid[self.uncertainty_grid.grid>0] = 1
            T = T_k.mean(-1)
            VH = self.uncertainty_grid(ray_pts)
            U_VH = segment_coo(
                src=(T * VH),
...
class Alphas2Weights(torch.autograd.Function):
    ...
    def forward(ctx, alpha, ray_id, N):
        ...
        if alpha.requires_grad:
            ...
            ctx.n_rays = N
        return weights, alphainv_last, T, i_end

    @staticmethod
    @torch.autograd.function.once_differentiable
    def backward(ctx, grad_weights, grad_last, __, _):
...
class Alphas2Weights_k(torch.autograd.Function):
    @staticmethod
    def forward(ctx, alpha_k, ray_id, N):
    # def forward(ctx, alpha_k, ray_id, N, alpha_std, alpha_std_sum):
        weights_k, T_k, alphainv_last_k, i_start, i_end, i_end_k = render_utils_cuda.alpha2weightkk(alpha_k, ray_id, N)
        # weights_k, T_k, alphainv_last_k, i_start, i_end, i_end_k = render_utils_cuda.alpha2weightkk(alpha_k, ray_id, N, alpha_std, alpha_std_sum)
        if alpha_k.requires_grad:
            ctx.save_for_backward(alpha_k, weights_k, T_k, alphainv_last_k, i_start, i_end, i_end_k)
            ctx.n_rays = N
        return weights_k, alphainv_last_k, T_k, i_end_k

    @staticmethod
    @torch.autograd.function.once_differentiable
    def backward(ctx, grad_weights_k, grad_last_k, __, _):
        alpha_k, weights_k, T_k, alphainv_last_k, i_start, i_end, i_end_k = ctx.saved_tensors
        grad_k = render_utils_cuda.alpha2weightkk_backward(
                alpha_k, weights_k, T_k, alphainv_last_k,
                i_start, i_end, i_end_k, ctx.n_rays, grad_weights_k, grad_last_k)
...
@torch.no_grad()
def get_training_rays_depth_flatten(rgb_tr_ori, depth_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    print('get_training_rays_depth_flatten: start')
    assert len(rgb_tr_ori) == len(train_poses) and len(rgb_tr_ori) == len(Ks) and len(rgb_tr_ori) == len(HW)
    eps_time = time.time()
    DEVICE = rgb_tr_ori[0].device
    N = sum(im.shape[0] * im.shape[1] for im in rgb_tr_ori)
    ...
    imsz = []
    top = 0
    for c2w, img, depth, (H, W), K in zip(train_poses, rgb_tr_ori, depth_tr_ori, HW, Ks):
        assert img.shape[:2] == (H, W)
        rays_o, rays_d, viewdirs = get_rays_of_a_view(
                H=H, W=W, K=K, c2w=c2w, ndc=ndc,
                inverse_y=inverse_y, flip_x=flip_x, flip_y=flip_y)
        n = H * W
        rgb_tr[top:top+n].copy_(img.flatten(0,1))
        depth_tr[top:top+n].copy_(depth.flatten(0,1).to(DEVICE))
        rays_o_tr[top:top+n].copy_(rays_o.flatten(0,1).to(DEVICE))
        rays_d_tr[top:top+n].copy_(rays_d.flatten(0,1).to(DEVICE))
        viewdirs_tr[top:top+n].copy_(viewdirs.flatten(0,1).to(DEVICE))
        imsz.append(n)
