...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    if not load_depths:
        return poses, bds, imgs, imgnames
    ...
    return poses, bds, imgs, imgnames, depths
...
def recenter_poses(poses):
    ...
    transform_m_recenter = np.linalg.inv(c2w)
    poses = np.linalg.inv(c2w) @ poses
    poses_[:,:3,:4] = poses[:,:3,:4]
    poses = poses_
    return poses, transform_m_recenter
...
def spherify_poses(poses, bds, depths):
    ...
    transform_m_spherify = np.linalg.inv(p34_to_44(c2w[None]))
    ...
    return poses_reset, sc, bds, depths, transform_m_spherify[0]
...
def load_llff_data(basedir, factor=8, width=None, height=None,
    ...
    poses, bds, imgs, imgnames, *depths = _load_data(basedir, factor=factor, width=width, height=height,
    ...
    if recenter:
        poses, transform_m_recenter = recenter_poses(poses)

    if spherify:
        poses, spherify_sc, bds, depths, transform_m_spherify = spherify_poses(poses, bds, depths)
        print('spherify sc: ',spherify_sc)

        if rerotate:
            poses, transform_m_rerotate, centroid = rerotate_poses(poses)
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    return images, depths, poses, bds, render_poses, sc
